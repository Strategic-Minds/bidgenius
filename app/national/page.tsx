  ...
  return (
    <div style={{ padding: '36px 40px', maxWidth: 1280, margin: '0 auto' }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <h1 style={{ margin: '6px 0', fontSize: 30, letterSpacing: -0.8 }}>National Contractor Operations</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.5fr) minmax(280px,0.7fr)', gap: 20, alignItems: 'start' }}>
        <section style={{ background: '#fff', border: '1px solid #E9E4D8', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ padding: '18px 20px', borderBottom: '1px solid #EEE9DF' }}>
            <h2 style={{ margin: 0, fontSize: 16 }}>50-State Registry</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 10, padding: 16 }}>
            {regions.map(region => {
              const states = nationalStates.filter((state: any) => state.region === region)
              return (
                <div key={region} style={{ border: '1px solid #F0ECE4', borderRadius: 12, padding: 14, background: '#FCFBF8' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <section style={{ background: '#fff', border: '1px solid #E9E4D8', borderRadius: 14, padding: 18 }}>
            ...
            {promotionRules.map((rule: string) => (
          ...
          <section style={{ background: '#fff', border: '1px solid #E9E4D8', borderRadius: 14, padding: 18 }}>
            ...
            {Object.entries(contractorSelectionWeights).map(([label, weight]) => (
              <div key={label} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#68635B', marginBottom: 4 }}>
                  <span>{label.replace(/([A-Z])/g, ' $1')}</span><b>{Math.round(Number(weight) * 100)}%</b>
                </div>
                <div style={{ height: 5, borderRadius: 999, background: '#EEEAE0', overflow: 'hidden' }}>
                  <div style={{ width: `${Number(weight) * 100}%`, height: '100%', background: GOLD }} />
                </div>
              </div>
            ))}
          </section>
          ...
          <section style={{ background: '#111', color: '#fff', borderRadius: 14, padding: 18 }}>
            ...
            <div style={{ color: '#D5D1C8', fontSize: 11, lineHeight: 1.7 }}>
